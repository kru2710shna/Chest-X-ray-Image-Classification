...
num_images = min(9, x_batch.shape[0])  # Display up to 9 images
...
for i in range(num_images):
    ax = axes[i]
    ax.imshow(x_batch[i])  # Display the image
    ax.axis('off')  # Hide axis
    ax.set_title(f"Label: {y_batch[i]}")  # Show label
...
def get_weighted_loss(pos_weights, neg_weights, epsilon=1e-7):
    ...
    def weighted_loss(y_true, y_pred):
        ...
        for i in range(len(pos_weights)):
            ...
            y_true_i = y_true[:, i]
            y_pred_i = y_pred[:, i]
            loss_pos = - pos_weights[i] * y_true_i * K.log(y_pred_i + epsilon)
            loss_neg = - neg_weights[i] * (1 - y_true_i) * K.log(1 - y_pred_i + epsilon)
